...
class Application(Frame):
    ...
    def make_pic(self):
        ...
        plot_data_array=self.current_up_row#self.current_row=pd.Series(self.current_row)这样子的程序没有用，无法改变类型
        ...
        if self.end_rowInput.get():
            ...
            if self.up_value.get()==1 and self.down_value.get()!=1:            
                for i in range(start_times+1,end_times+1):
                    ...
                    plot_data_array=np.concatenate((plot_data_array,self.current_down_row))#注意这里需要结合的两个array需要在一个括号里面
            if self.down_value.get()==1:
                for i in range(start_times+1,end_times+1):
        ...
        plot_data=pd.Series(plot_data_array)
        plot_data.plot()
        plt.show()
