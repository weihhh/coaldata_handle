...
def pre_proceed(data):
    ...
    data2=numpy.where(data1>pjz-2*bzc,data1,pjz)#两倍方差效果很好，三倍会有数据出错,一倍两倍差别不大    
    return data2
        ...
        if self.corr_multi.get()==1 :
            ...
            with open(r'D:\project\pypro\coaldata_handle\corr_all.csv','a',encoding='utf-8') as f:
                ...
                for i in range(start_times,end_times+1):
                    ...
                    u_data_corr=u_data_origin-numpy.max(u_data_pre)
                    d_data_corr=d_data_origin-numpy.max(d_data_pre)
